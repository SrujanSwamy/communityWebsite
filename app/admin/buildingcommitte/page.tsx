...
function ManageBuildingCommitteePage() {
  const [members, setMembers] = useState<BuildingCommitteeMember[]>([]);
  const [editingMember, setEditingMember] = useState<BuildingCommitteeMember | null>(null);
  ...
  const fetchMembers = async () => {
    ...
    if (memberError) {
      toast({
        title: "Error",
        description: "Failed to fetch building committee members",
        variant: "destructive",
      });
      return;
    } else {
  ...
  const handleAddMember = async () => {
    ...
    if (insertError) {
      toast({
        title: "Error",
        description: "Failed to add building committee member",
        variant: "destructive",
      });
      return;
    } else {
      toast({
        title: "Success",
        description: "Building committee member added successfully",
      });
    }
  ...
  const handleEditMember = async () => {
    ...
    if (updateError) {
      toast({
        title: "Error",
        description: "Failed to update building committee member",
        variant: "destructive",
      });
      return;
    } else {
      toast({
        title: "Success",
        description: "Building committee member updated successfully",
      });
    }
  ...
  const handleDeleteMember = async (id: number) => {
    ...
    if (deleteError) {
      toast({
        title: "Error",
        description: "Failed to delete building committee member",
        variant: "destructive",
      });
      return;
    } else {
      toast({
        title: "Success",
        description: "Building committee member deleted successfully",
      });
    }
    fetchMembers();
  };

  const openEditDialog = (member: BuildingCommitteeMember) => {
      ...
      <div className="container mx-auto">
        <Card className="bg-white border-2 border-[#B22222] mb-8">
          <CardHeader className="flex flex-row items-center justify-between">
            <CardTitle className="text-[#B22222]">Manage Building Committee</CardTitle>
            <Dialog open={isAddDialogOpen} onOpenChange={setIsAddDialogOpen}>
              ...
              <DialogContent className="sm:max-w-[425px]">
                <DialogHeader>
                  ...
                  <DialogDescription>
                    Add a new member to Building Committee
                  </DialogDescription>
                </DialogHeader>
        ...
        <Dialog open={isEditDialogOpen} onOpenChange={setIsEditDialogOpen}>
          <DialogContent className="sm:max-w-[425px]">
            <DialogHeader>
              ...
              <DialogDescription>
                Update building committee member information
              </DialogDescription>
            </DialogHeader>
            <div className="space-y-4 py-4">
              <div className="space-y-2">
...
export default ManageBuildingCommitteePage;